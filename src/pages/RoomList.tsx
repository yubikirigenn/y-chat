...
export default function RoomList({ session }: RoomListProps) {
  ...
  const fetchAllData = async () => {
    ...
    const { data: countsData, error: countsError } = await supabase.rpc('get_unread_counts_for_user');
    if (countsError) {
      console.error("Unread counts error:", countsError);
    } else if (countsData) {
      const countsMap = new Map((countsData as UnreadCount[]).map((item: UnreadCount) => [item.room_id, item.unread_count]));
      ...
      {isCreatingRoom ? (
        <div className="flex flex-col p-4 flex-1">
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={handleCreateGroupRoom} className="flex-1 px-3 py-2 bg-green-600 rounded">作成</button>
            <button onClick={() => setIsCreatingRoom(false)} className="flex-1 px-3 py-2 bg-gray-600 rounded">キャンセル</button>
          </div>
        </div>
      ) : (
        <main className="flex-1 overflow-y-auto">
            ...
            <ul>
              {rooms.filter(r => r.is_group).map(room => (
                <li key={room.id}>
                  <NavLink to={`/chat/${room.id}`} className={({ isActive }) => `flex items-center justify-between p-4 border-b border-gray-700 hover:bg-gray-800 ${isActive ? 'bg-blue-800' : ''}`}>
