...
function createMessageElement(msg) {
    ...
    item.innerHTML = `
        <img src="${msg.iconUrl}" class="message-avatar">
...
imageInput.addEventListener('change', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            socket.emit('chat message', { name: userName, imageUrl: result.imageUrl });
        } else {
...
socket.on('update user list', (users) => {
    ...
    others.forEach(user => {
        ...
        li.innerHTML = `<img src="${user.iconUrl}" class="user-avatar">${user.name}`;
...
socket.on('update rooms', (rooms) => {
    myRoomsInfo = {};
    rooms.forEach(room => { myRoomsInfo[room.name] = { isPrivate: room.isPrivate }; });
...
socket.on('join success', (data) => {
    ...
    if (unreadMessageIds.length > 0) {
        socket.emit('mark as read', { roomName: currentRoom, messageIds: unreadMessageIds });
    }
