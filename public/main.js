...
// --- イベントリスナー ---
createRoomBtn.addEventListener('click', () => createRoomDialog.showModal()); joinRoomBtn.addEventListener('click', () => joinRoomDialog.showModal()); cancelBtns.forEach(btn => { btn.addEventListener('click', () => { createRoomDialog.close(); joinRoomDialog.close(); }); });
...
// --- 初期化処理 ---
async function main() { initializeUserName(); initializeCredentials(); socket.emit('user connected', userName); }
main();

// --- 変更のない関数の再掲 ---
function renderRoomList(rooms, activeRoom) { roomList.innerHTML = ''; rooms.forEach(room => { const li = document.createElement('li'); li.dataset.room = room.name; li.dataset.isprivate = room.is_private; const nameSpan = document.createElement('span'); nameSpan.textContent = room.name; li.appendChild(nameSpan); if (unreadCounts[room.name] > 0) { const badge = document.createElement('span'); badge.className = 'unread-badge'; badge.textContent = unreadCounts[room.name]; li.appendChild(badge); } if (room.name === activeRoom) li.classList.add('active'); roomList.appendChild(li); }); }
...
roomList.addEventListener('click', (e) => { const li = e.target.closest('li'); if (li) { const roomName = li.dataset.room; if (roomName === currentRoom) return; const isPrivate = li.dataset.isprivate === 'true'; if (unreadCounts[roomName] > 0) { unreadCounts[roomName] = 0; const badge = li.querySelector('.unread-badge'); if (badge) badge.remove(); } if (isPrivate) { lastJoinAttempt = { roomName, password: null }; socket.emit('attempt join room', { roomName, password: null }); } else { let password = roomCredentials[roomName] || prompt(`'${roomName}' のパスワードを入力してください:`); if (password !== null) { lastJoinAttempt = { roomName, password }; socket.emit('attempt join room', { roomName, password }); } } } });
userList.addEventListener('click', (e) => { if (e.target.id === 'my-avatar') { iconInput.click(); return; } if (e.target.classList.contains('edit-name-btn')) { const oldName = userName; const newName = prompt('新しい名前を入力してください:', userName); if (newName && newName.trim() !== '' && newName !== userName) { socket.emit('change username', { oldName, newName: newName.trim() }); localStorage.setItem('chatUserName', newName.trim()); userName = newName.trim(); } return; } if (e.target.closest('li')) { const targetUserName = e.target.closest('li').dataset.username; if (targetUserName && targetUserName !== userName && confirm(`${targetUserName}さんと個人チャットを開始しますか？`)) { socket.emit('start private chat', targetUserName); } } });
...
socket.on('user icon changed', ({ userName: changedUserName, newIconUrl }) => { if (changedUserName === userName) { myIconUrl = newIconUrl; } socket.emit('request user list'); const avatars = document.querySelectorAll(`.message-item[data-sender-name="${changedUserName}"] .message-avatar`); avatars.forEach(avatar => { avatar.src = newIconUrl; }); });
socket.on('update user list', (users) => {
    userList.innerHTML = '';
