...
socket.on('update user list', (users) => {
    ...
    if (me) {
        ...
        // ★ BUG FIX: 正しいプロパティ名 icon_url を参照する
...
socket.on('update rooms', (rooms) => { /* ...変更なし... */ });
socket.on('join success', (data) => { /* ...変更なし... */ });
socket.on('join failure', (errorMessage) => alert(errorMessage));
socket.on('chat message', (msg) => { /* ...変更なし... */ });
socket.on('message deleted', ({ messageId }) => { /* ...変更なし... */ });
socket.on('update read status', ({ messageId, readers }) => { /* ...変更なし... */ });
