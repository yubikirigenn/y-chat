...
app.post('/upload-icon', async (req, res) => { const userName = req.body.userName; if (req.file && userName) { const newIconUrl = req.file.path; try { await db.query('UPDATE users SET icon_url = $1 WHERE name = $2', [newIconUrl, userName]); io.emit('user icon changed', { userName, newIconUrl }); res.json({ iconUrl: newIconUrl }); } catch(e) { console.error('アイコンURLのDB更新に失敗:', e); res.status(500).send('データベースエラー'); } } else { res.status(400).send('アップロード失敗'); } });
...
async function broadcastUserList() {
    try {
        // ★★★ "iconUrl" に統一 ★★★
        const usersResult = await db.query('SELECT name, icon_url AS "iconUrl" FROM users WHERE name = ANY($1::text[])', [Object.values(onlineUsers)]);
        io.emit('update user list', usersResult.rows);
    } catch(e) { console.error("ユーザーリストの取得に失敗:", e); }
}
async function sendRoomList(socket) { const userName = socket.userName; if (!userName) return; try { const myRoomsResult = await db.query("SELECT name, is_private FROM rooms WHERE participants @> $1", [`["${userName}"]`]); socket.emit('update rooms', myRoomsResult.rows); } catch(e) { console.error("ルームリストの取得に失敗:", e); } }
...
io.on('connection', (socket) => {
    ...
    socket.on('create room', async ({ roomName, password, creator }) => { await db.query('INSERT INTO rooms (name, password, creator, participants, is_private) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (name) DO NOTHING', [roomName, password, creator, JSON.stringify([creator]), false]); joinRoom(socket, roomName); sendRoomList(socket); socket.emit('join success', { roomName, history: [], isPrivate: false }); });
    ...
    socket.on('attempt join room', async ({ roomName, password }) => {
        const userName = socket.userName; const roomResult = await db.query('SELECT * FROM rooms WHERE name = $1', [roomName]);
        if (roomResult.rows.length === 0 || !userName) return socket.emit('join failure', '指定されたトークルームは存在しません。');
        const room = roomResult.rows[0]; const isAuthorized = room.is_private || room.creator === userName || room.password === password;
    ...
    socket.on('mark as read', async ({ roomName, messageIds }) => { const userName = socket.userName; if (!userName || !messageIds || messageIds.length === 0) return; try { for (const msgId of messageIds) { await db.query("UPDATE messages SET read_by = read_by || $1::jsonb WHERE id = $2 AND NOT (read_by @> $1::jsonb)", [JSON.stringify(userName), msgId]); const updatedMsgResult = await db.query('SELECT read_by FROM messages WHERE id = $1', [msgId]); if (updatedMsgResult.rows.length > 0) { io.to(roomName).emit('update read status', { messageId: msgId, readers: updatedMsgResult.rows[0].read_by }); } } } catch (e) { console.error('既読情報の更新に失敗:', e); } });
